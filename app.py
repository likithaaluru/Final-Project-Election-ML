...
def changepassword():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM people WHERE Id = %s', (session['Id'],))
        account = cursor.fetchone()
        ...
        if account['password'] == old_password:
            ...
            if new_password == confirm_password:
                ...
                cursor.execute('UPDATE people SET password = %s WHERE Id = %s', (new_password, session['Id']))
...
def sandy():
    if request.method == 'POST':
        try:
            ...
            pickle_in = open('vote1.pickle', 'rb')
            pac = pickle.load(pickle_in)
            tfid = open('tfid.pickle', 'rb')
            tfidf_vectorizer = pickle.load(tfid)
            total = query_content
            ...
            cur.execute("INSERT INTO review(news_content,pred,userid,parties_name) VALUES ( %s, %s,%s,%s) ",
                        (news_content, pred, Id, parties_name))
